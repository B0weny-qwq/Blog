---
title: Zephyr 入门（一）：认识 Zephyr 工程
slug: zephyr-lesson-1-project-overview
...
- 应用的 `CMakeLists.txt`：应用如何进入 Zephyr 构建系统。
- 应用的 `prj.conf`：应用请求启用哪些功能。
- 目标板的 `.dts`：板上有哪些硬件资源。
- 应用的 `src/main.c`：应用如何调用 Zephyr API。
- 构建生成的 `zephyr.dts`、`.config` 和 `devicetree_generated.h`：这次构建最终做了什么。
