...
以前学 RTOS，我基本也是跟着 Demo 创建几个任务，看到它们能来回切换，就觉得自己差不多会了。可一旦离开现成工程，问题马上就来了：内核到底要编哪些文件？`portable/` 里几十个目录该选谁？`FreeRTOSConfig.h` 那一大串宏又是干啥的？

所以这次先不急着逐行硬啃 `tasks.c`。对大多数嵌入式应用开发者来说，**先搞清楚移植层，比一上来钻进内核源码更重要**。

不是说 `tasks.c` 不值得看。真想往内核、编译器或者基础软件方向走，当然可以顺着调度器一路啃下去。但如果眼前目标只是让 F407 上的系统稳定跑起来，上来就钻进接近 9000 行的 `tasks.c`，大概率还没看到上下文切换，人先切换去刷视频了。
...
FreeRTOS 的核心确实是调度器，但 RTOS 不只是“同时跑很多任务”。它还要处理延时、超时、任务通信、资源互斥和中断唤醒这些事情。

像智能穿戴、机器人、飞控、仪器仪表这类系统，一边要采传感器，一边要跑控制，还得处理通信、日志和人机交互，RTOS 就很合适。PX4 跑在 NuttX 上也是这个道理：不是因为飞控只能做低频控制，而是整个系统除了快速控制环，还有大量不同周期、不同优先级的工作需要组织。

高精度伺服和电机驱动也不是绝对不能用 RTOS。真正要避开的是把最紧的电流环硬塞进一个普通任务，然后指望调度器提供微秒级确定性。快环可以留在定时器中断、PWM 更新点或者专用控制核里，RTOS 负责通信、状态机、参数管理和慢一些的控制环。工具没有错，关键是别让它去干不擅长的活。
...
所以这篇不准备逐行解析内核。先把仓库结构、文件作用、裁剪方法和 Cortex-M4F 移植边界讲清楚。源码可以慢慢啃，工程得先跑起来。
