...
现在回想起来，那时候对自己最准确的形容大概是：一个很能打的打手，但离工程师还差得远。对系统架构和文档开发的理解，几乎为零。
...
失败这种东西，本来就会反复砸在人头上。整个暑假我都闷闷不乐，也正是那股憋屈，逼着我继续往下学。

## 第二次进化：从开发到架构

后来我决定恶补 `HAL` 库。它更现代，我也开始意识到：面对陌生对象开发，封装 `API` 和整理文档不是装饰品，而是门槛。

于是我系统学了一套韦东山的 `HAL + RTOS`。第一次理解到 `OS` 的力量时，我甚至有点想哭。工具链也在那段时间一路革新：从 `Keil`，到 `CubeMX + CubeIDE`，再到 `CubeMX + CMake + Ninja`。

也是在那时候，`agent` 开始出现，我也逐渐把它引进开发流程。看了无数开源硬件和项目之后，我才第一次明白什么叫“工程”，也看清了过去很多活只是体力劳动披了一层技术外衣。
...
我很快意识到，重要的是架构规则本身。人虽然从重复劳动里解放出来了，但每天还是得负责架构层规划，以及技术栈的理论学习。毕竟这些东西你要是不懂，`review` 代码就是装样子，`debug` 也只能靠运气。
...
不过反过来看，尽管那时候已经在做架构相关的事，也还只是刚刚开智。别人未必做不到，只是很多人懒得往深处走，满足于把活干完。而我对技术要求更高，也一直想逼自己去做那些别人不愿做、或者根本做不了的活。
